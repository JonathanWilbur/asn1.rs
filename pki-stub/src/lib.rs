...
pub use crate::oids::{
    ...
    common_attr_type_to_short_name,
};
use std::str::FromStr;
use wildboar_asn1::{
    ASN1Error,
...
pub trait DisplayX500Value <OpenType> {
    ...
    fn value_to_string (self: &Self, attr_type: &OBJECT_IDENTIFIER, value: &OpenType) -> Result<Option<String>, ASN1Error>;
...
pub trait ParseX500Value <OpenType> {
    ...
    fn parse_value (&self, attr_type: &OBJECT_IDENTIFIER, s: &str) -> Result<Option<OpenType>, std::fmt::Error>;
...
pub fn parse_value <K: ParseX500AttributeType> (_k: &K, attr_type: &OBJECT_IDENTIFIER, s: &str) -> Result<Option<X690Element>, std::fmt::Error> {
    if s.starts_with("#") {
...
impl ParseX500Value<X690Element> for DefaultX500ValueParser {
    ...
    fn parse_value (&self, attr_type: &OBJECT_IDENTIFIER, s: &str) -> Result<Option<X690Element>, std::fmt::Error> {
