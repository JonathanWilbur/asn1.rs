...
use wildboar_asn1::{
    ASN1Value, BMPString, TagClass, UniversalString, INSTANCE_OF, UNIV_TAG_BMP_STRING, UNIV_TAG_PRINTABLE_STRING, UNIV_TAG_T61_STRING, UNIV_TAG_UNIVERSAL_STRING, UNIV_TAG_UTF8_STRING, read_i64
};
use x690::ber::BER;
use x690::{x690_write_tlv, X690Codec, X690Element};
use crate::{
...
fn replace_rfc_4514_specials (s: &str) -> Cow<str> {
    ...
    if owned_s.len() > 0 { // This only happens if we replaced a character.
        let len = s.len();
...
fn display_directory_string (
    ...
    match ds.tag.tag_number {
        ...
        UNIV_TAG_BMP_STRING => {
            let s = BER.decode_bmp_string(ds)
                .unwrap_or(BMPString::from_str("<?>").unwrap());
            s.fmt(f)
        },
        UNIV_TAG_UNIVERSAL_STRING => {
            let s = BER.decode_universal_string(ds)
...
fn write_unrecognized_value (f: &mut std::fmt::Formatter<'_>, value: &X690Element) -> std::fmt::Result {
...
#[cfg(test)]
mod tests {
    use crate::{GeneralName, Name, AttributeTypeAndValue};
    ...
    fn display_general_name_1() {
        ...
        assert_eq!(gn.to_string().as_str(), "directoryName:rdnSequence:dc=com,dc=wildboarsoftware");
    }
