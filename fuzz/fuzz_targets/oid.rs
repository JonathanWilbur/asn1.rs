...
fuzz_target!(|data: &[u8]| {
    let _ = asn1::OBJECT_IDENTIFIER::validate_x690_encoding(data);
    let oid1 = unsafe {
        asn1::OBJECT_IDENTIFIER::from_x690_encoding_slice_unchecked(data)
    };
    ...
    let oid2 = asn1::OBJECT_IDENTIFIER::from_prefix_and_arc(oid1.clone(), 1).unwrap();
    ...
    let roid = asn1::RELATIVE_OID::try_from([ 1u32, 5, 7 ].as_slice()).unwrap();
    let oid3 = asn1::OBJECT_IDENTIFIER::from_prefix_and_suffix(oid2.clone(), [ 5u32, 7 ].as_slice()).unwrap();
